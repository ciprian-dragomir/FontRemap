...
$method(FontRemap, 'stringEncode', function(stringToEncode, fontMap) {
    var res = [];
...
$method(FontRemap, 'encode', function(stringToEncode, fontMapPath, callback) {
    if(!callback) {
        return;
    }

    const self = this;
    fs.stat(fontMapPath, (err, stats) => {
        if(err) {
            throw err;
        }
        if(stats.isFile()) {
            const fontMap = JSON.parse(fs.readFileSync(fontMapPath, { encoding: 'utf8' }));
            callback.call(null, self.stringEncode(stringToEncode, fontMap));
        } else {
            throw {
                message: fontMapPath + ' is not a valid font mapping.',
